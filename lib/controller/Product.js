...
exports.GetDashboardCount = function(req,res)
{
    if (req.decoded.success == true) {
        connection.acquire(function (err, con) {
			...
			if(req.params.interval === 'Week')
			{
				var sql = 'SELECT companymaster.id,(SELECT COUNT(*) FROM user WHERE user.companyid = companymaster.id) as totalusers,(SELECT COUNT(*) FROM brand WHERE brand.companyid = companymaster.id) as totalbrands,(SELECT COUNT(*) FROM product WHERE product.companyid = companymaster.id) as totalproducts,(SELECT COUNT(*) FROM customermaster WHERE customermaster.companyid = companymaster.id) as totalcustomers,(SELECT COUNT(*) FROM inquiries WHERE inquiries.companyid = companymaster.id) as totalinquiries,IFNULL((SELECT SUM(ordermaster.netamount) FROM ordermaster WHERE ordermaster.companyid = companymaster.id AND WEEKOFYEAR(ordermaster.createddate) = WEEKOFYEAR(CURDATE())),0) AS totalorderamt,IFNULL((SELECT SUM(pomaster.netamount) FROM pomaster WHERE pomaster.companyid = companymaster.id AND WEEKOFYEAR(pomaster.createddate) = WEEKOFYEAR(CURDATE())),0) as totalpoamt FROM companymaster WHERE companymaster.id = ' + (req.decoded.logedinuser.companyid || 0)
			}
			if(req.params.interval === 'Month')
			{
				var sql = 'SELECT companymaster.id,(SELECT COUNT(*) FROM user WHERE user.companyid = companymaster.id) as totalusers,(SELECT COUNT(*) FROM brand WHERE brand.companyid = companymaster.id) as totalbrands,(SELECT COUNT(*) FROM product WHERE product.companyid = companymaster.id) as totalproducts,(SELECT COUNT(*) FROM customermaster WHERE customermaster.companyid = companymaster.id) as totalcustomers,(SELECT COUNT(*) FROM inquiries WHERE inquiries.companyid = companymaster.id) as totalinquiries,IFNULL((SELECT SUM(ordermaster.netamount) FROM ordermaster WHERE ordermaster.companyid = companymaster.id AND DATE_FORMAT(ordermaster.createddate,"%m-%Y") = DATE_FORMAT(CURDATE(),"%m-%Y")),0) AS totalorderamt,IFNULL((SELECT SUM(pomaster.netamount) FROM pomaster WHERE pomaster.companyid = companymaster.id AND DATE_FORMAT(pomaster.createddate,"%m-%Y") = DATE_FORMAT(CURDATE(),"%m-%Y")),0) as totalpoamt FROM companymaster WHERE companymaster.id = ' + (req.decoded.logedinuser.companyid || 0)
			}	
			if(req.params.interval === 'Year')
			{
				var sql = 'SELECT companymaster.id,(SELECT COUNT(*) FROM user WHERE user.companyid = companymaster.id) as totalusers,(SELECT COUNT(*) FROM brand WHERE brand.companyid = companymaster.id) as totalbrands,(SELECT COUNT(*) FROM product WHERE product.companyid = companymaster.id) as totalproducts,(SELECT COUNT(*) FROM customermaster WHERE customermaster.companyid = companymaster.id) as totalcustomers,(SELECT COUNT(*) FROM inquiries WHERE inquiries.companyid = companymaster.id) as totalinquiries,IFNULL((SELECT SUM(ordermaster.netamount) FROM ordermaster WHERE ordermaster.companyid = companymaster.id AND YEAR(ordermaster.createddate) = YEAR(CURDATE())),0) AS totalorderamt,IFNULL((SELECT SUM(pomaster.netamount) FROM pomaster WHERE pomaster.companyid = companymaster.id AND YEAR(pomaster.createddate) = YEAR(CURDATE())),0) as totalpoamt FROM companymaster WHERE companymaster.id = ' + (req.decoded.logedinuser.companyid || 0)
			}			
			if(req.params.interval === 'Today')
			{
				var sql = 'SELECT companymaster.id,(SELECT COUNT(*) FROM user WHERE user.companyid = companymaster.id) as totalusers,(SELECT COUNT(*) FROM brand WHERE brand.companyid = companymaster.id) as totalbrands,(SELECT COUNT(*) FROM product WHERE product.companyid = companymaster.id) as totalproducts,(SELECT COUNT(*) FROM customermaster WHERE customermaster.companyid = companymaster.id) as totalcustomers,(SELECT COUNT(*) FROM inquiries WHERE inquiries.companyid = companymaster.id) as totalinquiries,IFNULL((SELECT SUM(ordermaster.netamount) FROM ordermaster WHERE ordermaster.companyid = companymaster.id AND DATE_FORMAT(ordermaster.createddate,"%d-%m-%Y") = DATE_FORMAT(CURDATE(),"%d-%m-%Y")),0) AS totalorderamt,IFNULL((SELECT SUM(pomaster.netamount) FROM pomaster WHERE pomaster.companyid = companymaster.id AND DATE_FORMAT(pomaster.createddate,"%d-%m-%Y") = DATE_FORMAT(CURDATE(),"%d-%m-%Y")),0) as totalpoamt FROM companymaster WHERE companymaster.id = ' + (req.decoded.logedinuser.companyid || 0)
			}
...
exports.UploadProducts = function(req,res)
{
    if (req.decoded.success == true) {
        connection.acquire(function (err, con) {
            var ss = '';
                    req.body.map((value)=>{
                        ss += '("'+value.DTDS0+'","'+value.DTDS1+'","'+value.DTDS2+'",'+value.DTDS3+',' + req.decoded.logedinuser.id + ',' + (req.decoded.logedinuser.companyid || 0) + '),'
                    });
                    ss = ss.substring(0,ss.length -1);
          
            con.query('INSERT INTO product (name,description,slot,mrp,createdby,companyid) VALUES '+ss, function (err, result) {
                if (err) {
                    console.log(err)
                    res.send({
                        status: 1,
