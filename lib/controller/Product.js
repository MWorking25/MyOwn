...
exports.UpdateSaleOrder = function(req,res)
{
	 if (req.decoded.success == true) {
        ...
        connection.acquire(function (err, con) {
            ...
            var sql = 'UPDATE ordermaster SET ordermaster.paymentsts = "'+paymentsts+'", ordermaster.paidamount = '+req.body[0].paidamount+' WHERE ordermaster.id = '+orderdetails[0].masterid+';';
...
exports.getTimewaiseSale = function(req,res)
{
	  if (req.decoded.success == true) {
		...
		if(req.params.interval === 'Today')
		var sql = 'SELECT SUM(ordermaster.paidamount) as value,DATE_FORMAT(ordermaster.createddate,"%h") as category,DATE_FORMAT(ordermaster.createddate,"%h:%m:%s") as ordertime FROM `ordermaster` WHERE ordermaster.companyid = '+(req.decoded.logedinuser.companyid || 0)+' AND DATE_FORMAT(ordermaster.createddate,"%d-%m-%Y") = DATE_FORMAT(CURDATE(),"%d-%m-%Y") GROUP BY category';
	
		if(req.params.interval === 'Month')
		var sql = 'SELECT SUM(ordermaster.paidamount) as value,DATE_FORMAT(ordermaster.createddate,"%h") as category,DATE_FORMAT(ordermaster.createddate,"%h:%m:%s") as ordertime FROM `ordermaster` WHERE ordermaster.companyid = '+(req.decoded.logedinuser.companyid || 0)+' AND DATE_FORMAT(ordermaster.createddate,"%m-%Y") = DATE_FORMAT(CURDATE(),"%m-%Y") GROUP BY category'
		
		
		if(req.params.interval === 'Week')
		var sql = 'SELECT SUM(ordermaster.paidamount) as value,DATE_FORMAT(ordermaster.createddate,"%h") as category,DATE_FORMAT(ordermaster.createddate,"%h:%m:%s") as ordertime FROM `ordermaster` WHERE ordermaster.companyid = '+(req.decoded.logedinuser.companyid || 0)+' AND WEEKOFYEAR(ordermaster.createddate) = WEEKOFYEAR(CURDATE()) GROUP BY category'
	
		if(req.params.interval === 'Year')
		var sql = 'SELECT SUM(ordermaster.paidamount) as value,DATE_FORMAT(ordermaster.createddate,"%h") as category,DATE_FORMAT(ordermaster.createddate,"%h:%m:%s") as ordertime FROM `ordermaster` WHERE ordermaster.companyid = '+(req.decoded.logedinuser.companyid || 0)+' AND YEAR(ordermaster.createddate) = YEAR(CURDATE()) GROUP BY category'
...
exports.getMonthlysaleNPurchase = function(req,res)
{
	  if (req.decoded.success == true) {
				 connection.acquire(function (err, con) {
			 con.query('SELECT (SELECT SUM(orderdetails.qty) FROM orderdetails WHERE orderdetails.orderid = ordermaster.id) as saledqty,0 as poqty,MONTHNAME(ordermaster.createddate) AS saledmonth FROM ordermaster WHERE ordermaster.companyid = '+(req.decoded.logedinuser.companyid || 0), function (err, saleresult) {
                if (err) {
                    console.log(err)
                    res.send({
                        status: 1,
