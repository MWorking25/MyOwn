    ...
    if (req.decoded.success == true) {
        ...
        connection.acquire(function (err, con) {
		   ...
		   if(req.Loggedinuser.forgotpassword)
		     ...
		     var encpass = encrypt(req.body[0].password)
		   
		    con.query("UPDATE user SET password = ?,firstlogin = 1 WHERE id = "+ req.decoded.logedinuser.id,[encpass], function (err, result) {
				if(err)
				{
					res.send({
            status: false,
...
exports.ForgotPassword = function (userdetails, res) {
 connection.acquire(function (err, con) {
   con.query("SELECT id,email from user WHERE email = ?", [userdetails.email], function (err, result) {
    if (err) {
        res.send({
            success: false,
				type: "error",
...
exports.ValidateEmail = function (req, res) {
    if (req.decoded.success == true) {
        connection.acquire(function (err, con) {
            ...
            {
                var sql = "SELECT * FROM `user` WHERE id != "+req.params.userid+" AND email = '" + req.params.email + "'"
            }
...
exports.DeleteCompanyDetails = function (req, res) {
    if (req.decoded.logedinuser.role === "Superadmin" && req.decoded.success == true) {
        connection.acquire(function (err, con) {
            con.query("SELECT (SELECT COUNT(*) FROM user WHERE user.companyid = companymaster.id) as usersincomp,(SELECT COUNT(*) FROM product WHERE product.companyid = companymaster.id) as productscnt,(SELECT COUNT(*) FROM pomaster WHERE pomaster.companyid = companymaster.id) as poscnt,(SELECT COUNT(*) FROM ordermaster WHERE ordermaster.companyid = companymaster.id) as orderscnt,(SELECT COUNT(*) FROM inquiries WHERE inquiries.companyid = companymaster.id) as inquiriescnt,(SELECT COUNT(*) FROM customermaster WHERE customermaster.companyid = companymaster.id) as customerscnt,(SELECT COUNT(*) FROM brand WHERE brand.companyid = companymaster.id) as brandscnt  FROM `companymaster` WHERE id = " + req.params.companyid, function (err, result) {
                if (err) {
                    res.send({
                        status: 0,
                        type: "error",
...
exports.Listusers = function (req, res) {
    if (req.decoded.logedinuser.role === "Admin" && req.decoded.success == true) {
        connection.acquire(function (err, con) {
            con.query("SELECT *,(SELECT companymaster.name FROM companymaster WHERE companymaster.id = user.companyid) as companyname FROM `user` ORDER BY id DESC ", function (err, result) {
                console.log(err);
                if (err) {
                    res.send({
                        status: 1,
...
exports.ListUsersDetails = function (req, res) {
    if (req.decoded.logedinuser.role === "Admin" && req.decoded.success == true) {
        connection.acquire(function (err, con) {
            con.query("SELECT *,(SELECT companymaster.name FROM companymaster WHERE companymaster.id = user.companyid) as companyname FROM `user` WHERE user.companyid = "+(req.decoded.logedinuser.companyid || 0)+" ORDER BY id DESC ", function (err, result) {
                console.log(err);
                if (err) {
                    res.send({
                        status: 1,
...
exports.GetUserDetails = function (req, res) {
    if (req.decoded.logedinuser.role === "Admin" && req.decoded.success == true) {
        connection.acquire(function (err, con) {
            con.query("SELECT * FROM `user` WHERE id = "+req.params.userid, function (err, result) {
                console.log(err);
                if (err) {
                    res.send({
                        status: 1,
...
exports.SaveUserDetails = function (req, res) {
    if (req.decoded.logedinuser.role === "Admin" && req.decoded.success == true) {
        req.body.UserDetails.profilepic = req.file ? req.file.filename : '';
        req.body.UserDetails.createdby = req.decoded.logedinuser.id;
        connection.acquire(function (err, con) {
            if (!req.body.UserDetails.id) {
                        ...
                        con.query("INSERT INTO `user`(`name`, `mobile`,`email`,`role`,`companyid`,`createdby`,`profilepic`,`password`) VALUES (?,?,?,?,?,?,?,?)", [req.body.UserDetails.name, req.body.UserDetails.mobile, req.body.UserDetails.email,req.body.UserDetails.role, (req.decoded.logedinuser.companyid || 0), req.decoded.logedinuser.id, req.body.UserDetails.profilepic,encpass], function (err, result) {
                            console.log(err);
                            if (err) {
                                res.send({
                                    status: 1,
                                ...
                                const mailOptions = {
                                    ...
                                    subject: 'L.N. ', // Subject line
                                    html: 'Dear ' + req.body.UserDetails.name + '<br><br><br><h1 style="font-weight:bold;text-align:center;">' + passwordtxt + '</h1> <br> <p>enter this as a  password for the app.<br><br><br><br> <div style="float:left;text-align:left;">Thanks, <br> Admin <br> (L.N. software Pvt. Ltd.)</div></p>' // plain text body
                                };
            ...
            if (req.body.UserDetails.id) {
                con.query("UPDATE `user` set ? WHERE id = ?", [req.body.UserDetails, req.body.UserDetails.id], function (err, result) {
                    if (err) {
                        try {
                            fs.unlinkSync(req.file.path);
                            console.log('successfully deleted /tmp/hello');
...
exports.DeleteUserDetails = function (req, res) {
    if (req.decoded.logedinuser.role === "Admin" && req.decoded.success == true) {
        connection.acquire(function (err, con) {
            con.query("SELECT id,(SELECT COUNT(*) FROM ordermaster WHERE ordermaster.createdby = user.id) as ordercnt,(SELECT COUNT(*) FROM pomaster WHERE pomaster.createdby = user.id) as pocnt,(SELECT COUNT(*) FROM vendors WHERE vendors.createdby = user.id) as vendorcnt,(SELECT COUNT(*) FROM brand WHERE brand.createdby = user.id) as brandcnt,(SELECT COUNT(*) FROM product WHERE product.createdby = user.id) as prdcnt,(SELECT COUNT(*) FROM customermaster WHERE customermaster.createdby = user.id) as customerscnt,(SELECT COUNT(*) FROM inquiries WHERE inquiries.createdby = user.id) as inqcnt,(SELECT COUNT(*) FROM user a WHERE a.createdby = user.id) as usercnt FROM `user` WHERE user.id = " + req.params.userid, function (err, result) {
                if (err) {
                    res.send({
                        status: 0,
                        type: "error",
                    ...
                    if (result[0].ordercnt > 0 || result[0].pocnt > 0 || result[0].vendorcnt > 0 || result[0].brandcnt > 0 || result[0].prdcnt > 0 || result[0].customerscnt > 0 || result[0].inqcnt > 0 || result[0].usercnt > 0) {
                        res.send({
                            status: 0,
                            type: "error",
                            title: "Oops!",
                            message: "You can`t delete this user because, this user belongs with other records in system."
                        });
                    } else {

                        con.query("DELETE FROM `user` WHERE id = " + req.params.userid, function (err, result) {
                            if (err) {
                                res.send({
                                    status: 0,
                                    type: "error",
...
exports.saveUserActivities = function(req,res)
{
